...
interface AuthContextType {
  ...
  hasResume: boolean
  cacheUserData: (user: User | null, resumeMd: string | null) => void
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const cacheUserData = (user: User | null, resumeMd: string | null) => {
    try {
      ...
      setResumeMd(resumeMd)
      setHasResume(!!resumeMd?.trim())
    } catch (error) {
  ...
  const signUp = async (email: string, password: string) => {
    ...
    if (result.data.session && result.data.user) {
      ...
      setResumeMd(null)
      setLoading(false)
      // Remove immediate caching - will cache when resume is added
    }
...
export function updateCachedResume(resumeMd: string) {
  try {
	console.log("updating cahced resume")
    const cached = localStorage.getItem('atsfit_auth_state')
